function sizeLabel() {
    ...
    if (large.matches) { // If media query matches
      ...
      document.getElementById('label').innerHTML="Medium";
        ...
        showMenu();//parece que funciona

    }
  }
// adapte estas funciones para que aceptan parametros y asi reutilizar el codigo para las tres ciudades
  function getTempAndWind(id){
    const apiURL = `https://api.openweathermap.org/data/2.5/weather?id=${id}&appid=4b8335036073eddbe43ccf6d59157d39&units=imperial`;
    fetch(apiURL)
    ...
    function getForecast(id){
      const apiURL = `https://api.openweathermap.org/data/2.5/forecast?id=${id}&appid=4b8335036073eddbe43ccf6d59157d39&units=imperial`;
      fetch(apiURL)
        .then((response) => response.json())
        .then((jsObject) => {
  ...
  function verificaScreen(){
    let small = window.matchMedia("(min-width: 600px)");
    if (small.matches) {
      showMenu();
